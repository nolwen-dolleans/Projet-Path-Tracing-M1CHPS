...
L'équation de rendu se définie comme suit (cf démonstration en \hyperref[subsec:demo_rendering]{Annexe~\ref*{subsec:demo_rendering}}):
...
Ce type de matériaux inclus tous les matériaux de surface rugueuse qui n'émettent pas dans la lumière visible. Nous pouvons donc simplifier l'équation de rendu avec (cf démonstration \hyperref[subsec:demo_rendering]{Annexe~\ref*{subsec:demo_rendering}}):
...
Même avec cette équation simplifier, il est impossible de la résoudre analytiquement. C'est pour cela que nous allons utiliser une méthode stochastique pour pouvoir obtenir une solution convergeant vers la solution vraie: la méthode Monte-Carlo.\\

\subsection{Approximation avec la méthode Monte-Carlo}

La méthode de Monte-Carlo est une méthode itérative permettant de passer d'une intégrale à une somme d'échantillons aléatoires pondérée par une loi de probabilité notée $pdf$ (Probability Density Function). Elle sera ici utiliser pour approximer la lumière reçue en un point en remplaçant l’intégrale par une somme d’échantillons aléatoires, chacun pondéré par une loi de probabilité par la $pdf$.\\
...
Pour déterminer la direction des rayons aléatoires, nous choisissons un échantillonnage dit "Cosine-weighted". Il consiste à se placer dans le repère local orthonormée $(tangente, bitangente, normal)$ au point d'intersection (cf. \hyperref[fig:img_intersection]{figure~\ref*{fig:img_intersection}}), pour ensuite construire le rayon aléatoire.
...
\small{Source: \href{https://www.opengl-tutorial.org/fr/intermediate-tutorials/tutorial-13-normal-mapping/}{https://www.opengl-tutorial.org/fr/intermediate-tutorials/tutorial-13-normal-mapping/}
\label{fig:img_intersection}}
\end{figure}
...
\item[$\_$] Nous les passons en coordonnées polaires: $\left\{
\begin{array}{l}
r = \sqrt{\xi_1}\\
\theta = 2\pi\xi_2
\end{array}
\right.$
\item[$\_$] Nous passons les coordonnées polaires dans la base locale:$\left\{
\begin{array}{l}
x = r\cos\theta\\
y = r\sin\theta\\
z = \sqrt{1-r^2}
...
\end{itemize} 
Ainsi, on construit la direction du rayon incident $\omega_i$ de la manière suivante dans la base $\left( \vec{r_0}, \vec{u_0}, \vec{d_0}\right)$: $\omega_i = x\cdot tangente + y\cdot bitangente+ z\cdot normal $.

L'avantage de cette méthode d'échantillonnage est que la $pdf$ sera: $pdf = \dfrac{\cos\theta_i}{\pi}$. C'est pour cette raison qu'elle est majoritairement utilisée dans le Path Tracing.\\
...
Pour implémenter l'équation de rendu, nous allons utiliser la parité de la radiance (c'est-à-dire $\, L(x,\omega) = L(x,-\omega)$) afin de propager les rayons de la caméra vers la source de lumière plutôt que l'inverse, qui serait le phénomène physique naturel.
