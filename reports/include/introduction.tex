...
Le ray casting est une technique de rendu visuel qui a pour principe d'afficher des images avec une illusion de profondeur / 3D. Cette technique a été popularisée par le studio ID Software, qui l'a entre autre utilisé pour Wolfenstein 3D, DOOM et Quake. 
Elle fut souvent utilisée pour du temps réel, car elle était très peu coûteuse en termes de calcul pour les machines de l'époque.

L'environnement est représenté de la manière suivante: une caméra à partir duquel on balance un rayon par pixel. Pour chaque pixel tracer, on calcule l'intersection de chaque objet toucher par le rayon. Ensuite, on affiche le pixel avec la distance la plus petite.

Cette technique apporte quelques inconvénients, d'où le fait qu'elle soit utilisée en complément d'autres techniques de rendu. 
Elle ne permet pas d'effectuer du rendu sur des objets 3D complexes. Pour du temps réel, nous sommes limité sur une rotation par rapport à l'axe de Y. 
Nous ne pouvons pas faire du rendu volumétrique, i.e rendu d'un effet de flamme ou liquide. Pour du temps réel, nous sommes limité sur une rotation par rapport à l'axe de Y. 
On ne gère pas les rebonds.
...
De nos jours, la technique de rendu la plus populaire est la rasterization. Bien que moins précise visuellement, la rasterization demeure dominante dans les moteurs temps réel.
...
Le Path-Tracing comprend de plus un test pour d'éventuelles intersections de chemins lumineux, ce qui demande des performances, particulièrement pour des modèles 3D complexes.
