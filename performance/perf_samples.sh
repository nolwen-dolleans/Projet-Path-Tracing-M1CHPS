...
if [ -f "performance/measures/mpi_samples.json" ]; then
    rm "performance/measures/mpi_samples.json"
fi
OUTPUT_FILE="performance/measures/mpi_samples.json"

mkdir -p "$(dirname "$OUTPUT_FILE")"

echo "[" > "$OUTPUT_FILE"

# samples values to test
SAMPLES=(50 100 150 200) 

for i in "${!SAMPLES[@]}"; do
    ITER=${SAMPLES[$i]}

    echo "Running MPI program with samples=$ITER..."

    # Collect CPU cycles, instructions, cache references, cache misses, memory usage
    PERF_OUTPUT=$(BOUNCES=2 perf stat -e cycles,instructions,cache-references,cache-misses -o /dev/stdout -- \
        /usr/bin/time -v mpirun -np 8 ./ppm 32 800 600 $ITER 2>&1)
